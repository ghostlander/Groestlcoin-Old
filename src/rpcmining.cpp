...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "GroestlCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "GroestlCoin is downloading blocks...");
...
Value getworkaux(const Array& params, bool fHelp) {
    ...
    if(vNodes.empty())
      throw(JSONRPCError(-9, "Wallet is not connected!"));

    if(IsInitialBlockDownload())
      throw(JSONRPCError(-10, "Wallet is downloading blocks..."));
