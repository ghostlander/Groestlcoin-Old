...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GroestlCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GroestlCoin
    // Mac: ~/Library/Application Support/GroestlCoin
    // Unix: ~/.GroestlCoin
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "GroestlCoind.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 14 * 60) // GroestlCoin: changed maximum adjust to 14 mins to avoid letting peers change our time too much in case of an attack. (same as in DarkCoin)
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong GroestlCoin will not work properly.");
