...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GRS unit input in lnReqAmount, we generate the URI with GRS as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::GRS, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
